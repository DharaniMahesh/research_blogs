...
interface MetaPost extends Post {
    category?: string;
}
    ...
    const fetchPosts = useCallback(async (pageNum: number, isInitial: boolean = false) => {
        ...
        try {
            const response = await fetch(
                `/api/fetch?sourceId=meta-engineering&page=${pageNum}&maxPosts=20`
            );

            if (!response.ok) {
                throw new Error('Failed to fetch Meta posts');
            }
            ...
            setPosts(prev => {
                ...
                // Deduplicate by URL
                const seen = new Set();
                return combined.filter((post: MetaPost) => {
            ...
            setHasMore(data.hasMore && newPosts.length > 0);
            setPage(pageNum);
        } catch (err) {
    ...
    return (
        <div className="min-h-screen bg-gradient-to-br from-slate-950 via-slate-900 to-blue-950">
            ...
            <header className="relative overflow-hidden border-b border-slate-800/50 bg-slate-950/50 backdrop-blur-xl sticky top-0 z-50">
                <div className="absolute inset-0 bg-gradient-to-r from-blue-500/10 via-indigo-500/10 to-blue-500/10" />
                <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
                    <div className="flex items-center justify-between">
                        <div className="flex items-center gap-4">
                            ...
                            <div>
                                <h1 className="text-2xl md:text-3xl font-bold bg-gradient-to-r from-blue-400 via-indigo-400 to-cyan-400 bg-clip-text text-transparent">
                                    Engineering at Meta Hub
                                </h1>
                                <p className="text-slate-400 mt-1 text-sm md:text-base">
                                    Latest engineering blogs, publications, and research from Meta
                                </p>
                            </div>
                        </div>
            ...
            <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
                ...
                <div className="flex flex-col sm:flex-row gap-4 mb-8">
                    ...
                    <div className="relative flex-1">
                        ...
                        <input
                            type="text"
                            placeholder="Search Meta engineering..."
                            value={searchQuery}
